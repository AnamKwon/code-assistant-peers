...
- 同一状態 dedup: repo 状態とオプションが直前の完了レビューと同じならトークンを使わず記録済みレビューを返す（`force_review` で再実行）
...
レビューは Agent SDK クレジットプールではなく**サブスクリプションプール**に課金されます。
...
`codex,claude-live`）。詳細な設定と課金確認チェックリストは
...
| `CODE_ASSISTANT_PEERS_REVIEW_MODEL` | unset | host が review_model を渡さない場合の既定（`auto` 推奨: 小さい diff を低コストモデルへ自動ルーティング） |
| `CODE_ASSISTANT_PEERS_MEMORY_ROUNDS` | `3` | prompt にインライン展開する直近レビューラウンド数（未解決 findings は常に全件含む） |
| `CODE_ASSISTANT_PEERS_REVIEWER_CLEAR` | keep | ライブセッションの会話メモリ既定は維持。`always` でレビューごとに初期化 |
