...
- Claude Code / Codex / Gemini の組み込み adapter
- デフォルトの peer routing は `claude-live`, `codex-live`, `gemini-live` を優先します。
- Claude のレビューをバックグラウンドの対話型 Claude セッションに送る `claude-live` adapter（サブスクリプションプール課金、`claude -p` 不使用、自動起動 + `claude -p` fallback）
...
4 つ目の組み込み adapter である `claude-live` は、`claude -p` を起動せず localhost broker 経由で
**バックグラウンドの対話型 Claude セッション**にレビューを送ります。これにより Claude の
レビューは**サブスクリプションプール**に残ります。2026-06-15 以降、`claude -p` / Agent SDK
レビューは別の programmatic/API クレジットプールを使います。
broker と reviewer worker は最初の `claude-live` レビュー時に自動起動し、セッションは
リポジトリ単位の read-only で、broker/セッション障害時は `claude -p` の起動に fallback します。
peer に `claude` を指定していた場所でそのまま使えます（`PEER_ASSISTANTS=claude-live` または
`codex-live,claude-live`）。詳細な設定と課金確認チェックリストは
[broker/REVIEWER.md](../broker/REVIEWER.md) を参照してください。
...
メモリ**です（`CODE_ASSISTANT_PEERS_REVIEWER_CLEAR=never`）。self-review や aggregate などの host 側パスも
既定で host の live adapter を使います。headless host 経路を強制するには `CODE_ASSISTANT_PEERS_LIVE_HOST_REVIEWS=0` を設定します。self-review の対象は
`CODE_ASSISTANT_PEERS_SELF_REVIEW`（既定 `codex` / `all` / `none` / `claude,codex` など）で指定します。

他の CLI は `CODE_ASSISTANT_PEERS_ASSISTANTS` に JSON で登録します。`prompt_transport` は
`stdin`, `argv`, `channel` のいずれかです（`channel` は command を起動せず live-reviewer broker
にレビューを送ります）。
...
| `CODE_ASSISTANT_PEERS_LIVE_HOST_REVIEWS` | live by default | `0`, `false`, `no`, `off` で host-side pass を headless CLI に強制 |
| `CODE_ASSISTANT_PEERS_ASSISTANTS` | built-ins only | custom CLI adapter JSON |
| `CODE_ASSISTANT_PEERS_PROBE_MODELS` | unset | `1` で known model candidates を小さな live probe で確認 |
| `CODE_ASSISTANT_PEERS_MODEL_PROBE_TIMEOUT_MS` | `30000` | model probe timeout |
| `CODE_ASSISTANT_PEERS_DEV_LOG` | unset | `1` で broker jobs、`requestedModel`/`usedModel`、tmux resume、marker extraction の JSONL log を記録 |
| `CODE_ASSISTANT_PEERS_DEV_LOG_PATH` | `<cwd>/.code-assistant-peers-dev/<scope>.jsonl` | developer JSONL log path。既定ディレクトリは gitignore 済み |
...
| `CODE_ASSISTANT_PEERS_SERENA_CONTEXT_BUDGET` | `8000` | Serena semantic context character budget |
